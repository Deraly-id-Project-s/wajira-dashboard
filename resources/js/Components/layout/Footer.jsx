  ...
  export default function Footer() {
    const { data, loading, error } = useFetchData("/api/links");

    const socialLinks = data?.status
    ? data.data.filter((item) => item.is_show === 1)
    : [];
        ...
        <div className="border-t border-slate-600">
          <div className="max-w-7xl mx-auto px-4 py-6 flex flex-col md:flex-row justify-between items-center text-xs text-gray-300 space-y-4 md:space-y-0">
            ...
            <div className="flex space-x-4 text-white font-semibold">
              {getUrl("WhatsApp") && (
                <a href={`https://wa.me/${getUrl("whatsapp")}`} target="_blank" rel="noopener noreferrer">
